...
- `COPY package*.json ./`, esta instrucción copia todos los archivos que coinciden con `package*.json` que se encuentran en la raíz de nuestro proyecto de angular, es decir, aquellos archivos que inician con `package` y terminan con `.json`. En nuestro caso tenemos solo dos archivos que coinciden: el `package.json` y el `package-lock.json`. Estos archivos serán copiados en el directorio de trabajo `/app`, es decir, cuando coloquemos el `./` o solo el `.` **estaremos haciendo referencia al directorio de trabajo que creamos en la instrucción anterior**, el `/app`.

  Ahora, **¿por qué copiamos primero estos dos archivos y no todo el código fuente?** es porque queremos instalar todo la primera vez. Recordemos que en el archivo `package.json` tenemos todas las dependendecias que usa nuestro proyecto de angular, por lo tanto, queremos que la primera vez podamos descargar todas sus dependencias y `no cada vez que nuestro código fuente cambie`. La próxima vez que cambiemos nuestro código y construyamos la imagen, Docker usará las "capas" almacenadas en caché con todo lo instalado (porque el **package.json** no ha cambiado) y solo compilará nuestro código fuente.

- `RUN npm install`, iniciamos la instalación de todas las dependencias definidas en el archivo `package.json`. Esto se almacenará en caché hasta que cambiemos el archivo package.json (cambiando nuestras dependencias). Por lo tanto, no tomará mucho tiempo instalar todo cada vez que iteremos en nuestro código fuente e intentemos probar (o implementar) la imagen de producción de Docker, solo la primera vez y cuando actualicemos las dependencias (paquetes instalados)
...
- `COPY . .`, después de instalar todas las dependencias, podemos copiar nuestro código fuente, entonces le decimos que copie todos los archivos y directorios que estén en la misma raíz que el `Dockerfile` (en los subdirectorios también) y los pegue dentro del working directory `/app`. El segundo punto `.` al igual que usamos el `./` en una instrucción superior, hacen referencia al working directory `/app`.
...
Recordemos que **a partir de la versión 12 de Angular** cuando ejecutamos el comando `ng build` la aplicación `se construye para un entorno de producción de forma predeterminada`. 

Ahora, si queremos ser explícitos en el tipo de entorno que queremos generar el proyecto, podemos colocar la bandera `--configuration` y si lo queremos para un `entorno de producción` su valor sería `production`.

En nuestro caso, siguiendo con el tutorial, coloca la bandera `--configuration production` en el script `build` para hacer explícita la creación del proyecto para producción.
